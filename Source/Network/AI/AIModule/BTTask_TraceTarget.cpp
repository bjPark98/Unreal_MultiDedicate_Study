...
UBTTask_TraceTarget::UBTTask_TraceTarget()
{
	...
	//UE_LOG(Network, Warning, TEXT("Task"));
}
	...
	uint8* NodeMemory)
{
	...
	AMonsterPawn* Pawn = Cast<AMonsterPawn>(Controller->GetPawn());
	...
	if (!IsValid(Target))
	{
		...
		// 애니메이션을 Idle 로 변경한다.
		Pawn->ChangeAnimType((uint8)EMonsterAnimType::Idle);
	...
	// 애니메이션을 Run 으로 변경한다.
	Pawn->ChangeAnimType((uint8)EMonsterAnimType::Run);
